        ...
        page_header(
            ...
            filter_bar(
                field(
                    ...
                    control: "number",
                    class: "w-40",
                    input(attrs: topcoat::view::attributes! { id="number" name="number" placeholder="F2026-" autocomplete="off" value=(query.number.clone().unwrap_or_default()) })
                )
                field(
                    label: "Statut",
                    control: "status",
...
async fn invoice_row(
    ...
    Ok(view! {
        table_row(
            ...
            table_cell(link(href: target, class: "font-mono text-xs", (number)))
