//! `/{mount}/orders`: every order, newest first, filterable by status.
...
pub async fn index(cx: &Cx) -> Result<impl View> {
    ...
    .await?;
    let total = count_orders(db, status).await?;
...
async fn order_row(cx: &Cx, row: &OrderHistoryRow) -> Result<impl View> {
    ...
    Ok(view! {
        table_row(
            ...
            table_cell(<a href=(link) class="font-mono text-xs underline-offset-4 hover:underline">(row.order_id.clone())</a>)
